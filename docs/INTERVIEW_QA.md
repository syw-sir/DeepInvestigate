...
**回答**：
Critic 审查后返回的 `critic_feedback` 包含 `issues` 和 `suggestions` 两个字段。回退时，我在 `_route_after_critic` 返回 `"retry"`，图自动跳回 `investigator` 节点。
...
3. **迭代上限**：`max_iterations=15` 防止无限跑偏
...
`interrupt()` 是**运行时动态调用**，在节点函数内部按条件触发。我可以拿到当前 state，检查 tool_calls 的具体工具名，查 `TOOL_RISK_LEVELS` 表，只有 risk >= 1 才调用 `interrupt()`。
...
**回答**：
链路如下：
1. 图执行到 `_human_approval_node`，调用 `interrupt({tool_name, risk_level, ...})`
2. LangGraph 将 interrupt 的值序列化存入 checkpoint，图暂停，抛出 `GraphInterrupt` 异常
3. 前端收到 SSE 中的 `on_chain_interrupt` 事件，展示审批卡片
4. 用户点击批准/拒绝，前端调 `POST /chat/resume`，传 `{thread_id, approved: true/false}`
5. 后端构造 `Command(resume={"approved": True})`，调用 `graph.astream_events(cmd, config)`
6. LangGraph 从 checkpoint 恢复状态，将 `Command.resume` 的值作为 `interrupt()` 的返回值
7. `_human_approval_node` 拿到返回值，判断 `result.get("approved")`，决定放行还是拒绝

关键点：`interrupt()` 的返回值 = `Command(resume=...)` 的 resume 参数。这是 LangGraph 的约定，不是魔法。
...
**回答**：
`SqliteSaver` 存的是 LangGraph 的完整状态快照：每个节点执行后的 `AgentState`（messages、plan、iteration_count、所有 v3.0 字段）、当前待执行的节点列表、以及元数据（时间戳、step 编号）。
...
### 🟢 Q7: 你的 9 个工具是怎么注册到 LangGraph 的？新增一个工具需要改哪些地方？
...
新增工具的步骤：
1. 在 `tools/` 下新建 `my_tool.py`，用 `@tool` 定义函数
2. 在 `tools/registry.py` 的 `get_all_tools()` 中 `import` 并加入列表
3. 如需 HITL 审批，在 `graph.py` 的 `TOOL_RISK_LEVELS` 中加一条
...
### 🟡 Q8: 工具调用失败（比如 API 超时、沙箱执行异常）你是怎么处理的？Agent 会崩溃吗？

> **面试官意图**：考察容错设计。

**回答**：
不会崩溃，有三层容错：
1. **工具层**：每个工具内部有 try/except，异常时返回错误信息字符串而非抛异常，Agent 看到的是 "Error: API timeout after 30s" 这样的 ToolMessage
2. **图层面**：`tool_executor` 节点（ToolNode）本身有异常处理，工具返回的错误消息会正常追加到 messages
3. **Agent 层**：Investigator 的 system prompt 里有"如果工具返回错误，分析原因并尝试替代方案"，它会自动换工具或调整参数

实际效果：Agent 看到工具报错后会尝试换关键词重试、换工具、或跳过该步骤并在报告中标注"无法验证"。不会因为一次工具失败就整个任务崩掉。

---

### 🔴 Q9: 你的代码沙箱说"默认禁网"，具体怎么实现的？能绕过吗？

> **面试官意图**：考察安全意识的深度。

**回答**：
当前实现是**进程级软隔离**，不是真沙箱：
1. 静态黑名单正则拦截 `import socket`、`urllib`、`requests`、`http.client` 等
2. 子进程用 `subprocess.run(..., cwd=workspace_dir)` 限制工作目录
3. 超时 + 输出截断

**已知绕过方式**（诚实承认）：
- `__import__('so'+'cket')` 字符串拼接绕过正则
- `exec(bytes.fromhex('...'))` 十六进制编码绕过
- 利用已有库的副作用（如 pandas 的 `read_csv('http://...')`）
...
**回答**：
拒绝后，`_human_approval_node` 返回一条 AssistantMessage："操作已被用户拒绝: run_python"。然后 `_route_after_approval` 返回 `"skip"`，图跳回 `investigator`。

Investigator 看到这条消息后，system prompt 指示它"如果操作被拒绝，尝试替代方案或跳过该步骤"。实际行为：
- 如果是代码执行被拒，它会尝试用纯文本推理替代
- 如果是文件写入被拒，它会在报告中标注"用户拒绝了文件生成请求"
- 不会死循环请求同一个工具，因为拒绝消息在上下文中

额外保护：`max_iterations` 硬限制，即使 Agent 犯傻也不会无限跑。

---

### 🟡 Q11: 你的 PII 脱敏是正则做的，遇到"我的手机号是 13812345678"能脱敏，但遇到"联系方式：一三八一二三四五六七八"呢？

> **面试官意图**：考察对正则方案局限性的认知。

**回答**：
正则确实处理不了中文数字、谐音、图片中的 PII。这是已知局限。

分层策略：
1. **正则层**（当前）：覆盖标准格式，拦截 90%+ 常见场景
2. **可扩展**：`OutputGuard.sanitize()` 接口设计为可插拔，后续可以接入 NER 模型（如 bert-base-chinese-ner）识别中文数字变体
3. **兜底**：敏感场景建议在 LLM 输出前加一层 `presidio` 或云端 DLP

面试中诚实说"正则覆盖标准格式，中文变体需要 NER 模型"比吹嘘"全覆盖"更可信。
...
### 🔴 Q14: 你的 RAG 用 BGE-M3 做 embedding，为什么选它而不是 OpenAI embedding？如果知识库从 1000 条涨到 100 万条，ChromaDB 还撑得住吗？

> **面试官意图**：考察技术选型的思考深度和扩展性意识。

**回答**：
选 BGE-M3 的原因：
1. **成本**：本地跑免费用，OpenAI embedding 100 万条要几十美元
2. **多语言**：BGE-M3 中英文都好，安全领域中文资料多
3. **可控**：不依赖外部 API，离线也能跑

100 万条的扩展问题：
- ChromaDB 单机在 100 万条时查询延迟会从 10ms 升到 100ms+，还能用但不再丝滑
- 扩展路径：换 Milvus/Qdrant 分布式向量库，或 ChromaDB 加多段索引 + 量化压缩
- 更根本的方案：加**检索路由**——先关键词过滤缩小候选集，再向量检索，减少计算量

---

### 🟡 Q15: 你的四层记忆和 Mem0 是什么关系？为什么不直接用 Mem0 替代全部？
...
### 🔴 Q16: 你的评估体系里"幻觉检测"怎么做的？LLM 输出"根据 CVE-2024-1234，该漏洞影响 Linux 内核 5.15"你怎么验证真假？
...
你说的 CVE 验证属于**事实级幻觉检测**，当前做不到。这需要：
- 外部知识库校验（查 CVE 数据库确认该编号是否存在、影响范围是否匹配）
- 或用一个独立的 FactCheck Agent 逐条验证

诚实说：当前评估体系的幻觉检测是弱检测，只能抓明显问题，不能做事实核查。这是评估体系最大的待改进点。

---

### 🟡 Q17: 你的评估数据集只有 15 条，怎么保证统计显著性？会不会过拟合？
...
防过拟合：
- 测试集是我自己写的，但写的时候尽量覆盖不同类别（日志分析/漏洞调查/攻击溯源/威胁情报/合规检查）
...
2. **Python 沙箱**：`subprocess.run` 是同步阻塞的，asyncio 线程池有限，大量代码执行会排队
...
**回答**：
当前是 Docker Compose 一键启动：`docker-compose up -d` 拉 Redis + ChromaDB，然后 `python server_v2.py` 启 API。
...
### 🔴 Q20: MCP 和 A2A 都是 Google 提出的 Agent 协议，你项目中两个都用了，它们分别解决什么问题？什么场景用哪个？

> **面试官意图**：考察对行业前沿协议的理解深度。

**回答**：
- **MCP（Model Context Protocol）**：解决 **Agent ↔ 工具** 的通信标准。让工具可以被任何 Agent 框架调用，类似"工具的 USB-C 接口"。我的项目里用它把 9 个工具暴露为 MCP Server，外部 Agent 也能用。
- **A2A（Agent-to-Agent）**：解决 **Agent ↔ Agent** 的通信标准。定义 Agent 之间怎么发任务、传结果、请求澄清。我的项目里用它做多 Investigator 并行调查。
...
### 🟡 Q21: 现在 Agent 框架这么多（LangGraph、CrewAI、AutoGen、Dify、Coze），你为什么选 LangGraph？什么场景下你会选别的？

> **面试官意图**：考察技术选型的成熟度和行业视野。

**回答**：
选 LangGraph 的原因：
1. **可控性**：显式图结构，每个节点和边我都能控制，不像 CrewAI 的 Sequential Process 是黑盒
2. **生态**：LangChain 工具链 + LangSmith 追踪 + Checkpointer 持久化，全家桶开箱即用
3. **灵活性**：支持 interrupt、Command、动态路由，能做复杂的人机协作流程

什么场景选别的：
- **快速原型**：Dify/Coze 拖拽式，非技术人员也能用，适合 PoC
- **简单多 Agent 对话**：CrewAI 的 Role-Based Agent 更直观，代码量更少
- **微软生态**：AutoGen 和 Azure/Windows 集成更好
- **超大规模**：LangGraph 的 Python 实现有 GIL 限制，极端并发可能需要 Go/Rust 方案
...
### ⚫ Q22: 你说 Critic 评分 < 4 就回退，但如果你的 Critic 本身就很蠢怎么办？它给好报告打低分、给烂报告打高分，整个系统不就崩了？
...
当前缓解措施：
1. **硬上限**：最多回退 2 次，防止 Critic 误判导致无限循环
2. **低温度**：Critic 用 temperature=0.2，减少随机性
3. **结构化输出**：要求 JSON 格式，减少解析歧义

根本解决方案：
- **Critic 校准**：用人工标注的"好报告/烂报告"对 Critic 做准确率测试，低于 80% 不启用
- **双 Critic**：两个不同模型独立评分，取均值，减少单模型偏差
- **人工抽检**：定期人工复查 Critic 的评分是否合理

坦白说，这是整个系统最脆弱的一环。如果面试官追问"你测过 Critic 的准确率吗"，答案是"还没有系统测过，这是下一步要做的事"。

---

### ⚫ Q23: 你的整个项目跑起来要多少钱？Redis Stack + ChromaDB + DeepSeek API + 前端，一个月不关机多少钱？
...
1. **不是 Demo，是系统**：4 Agent 协作 + 9 工具 + 4 层记忆 + RAG + 评估体系，每个模块都能独立展开讲
2. **有生产级特性**：HITL 审批、Checkpointer 持久化、多模型 fallback、PII 脱敏、审计日志——这些不是"调个 API 就能做"的
3. **踩过坑有思考**：`interrupt_before` vs `interrupt()` 的选择、Critic 回退的防无限循环、沙箱安全的局限性——每个设计决策都有 trade-off 分析
